# Description: This file contains the functions to read and write the TSV file with the metadata of the tracks.
...
def read_file(tsv_file):
    ...
    with open(tsv_file) as fp:
        ...
        for row in reader:
            ...
            tracks[track_id].update({category: set()
    ...
    print("Reading: {} tracks, {} albums, {} artists".format(
...
def write_file(tracks, tsv_file, extra):
    ...
    for track_id, track in tracks.items():
        row = [
            ...
            'artist_' + str(track['artist_id']
                            ).zfill(extra['artist_id_length']),
    ...
    with open(tsv_file, 'w') as fp:
        ...
        writer.writerow(['TRACK_ID', 'ARTIST_ID', 'ALBUM_ID',
                        'PATH', 'DURATION', 'TAGS'])
        for row in rows:
            writer.writerow(row)
